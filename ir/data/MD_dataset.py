...
class MDDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # self.lq_cls = "D:\\ZQ\\hazy\\trainLQ_cls"
        # self.lq_cls_path = []
        # self.test_lq_cls = "D:\\ZQ\\hazy\\testLQ_cls"
        # self.test_lq_cls_path = []

        # for root,dirs,files in os.walk(self.lq_cls):
        #     for file in files:
        #             self.lq_cls_path.append(os.path.join(root, file))
        # for root,dirs,files in os.walk(self.test_lq_cls):
        #     for file in files:
        #             self.test_lq_cls_path.append(os.path.join(root, file))
        self.distortion = {}
    ...
    def __getitem__(self, index):
        ...
        # #get lq cls
        # lq_cls_path = self.lq_cls_path[index]
        # lq_cls_embedding = torch.load(lq_cls_path)
        # #get test lq cls
        # test_lq_cls_path = self.test_lq_cls_path[index]
        # test_lq_cls_embedding = torch.load(test_lq_cls_path)
        if self.opt["phase"] == "train":
            H, W, C = img_GT.shape
        ...
        #     rnd_h = 600
        #     rnd_w = 800
        #     img_GT = img_GT[rnd_h: rnd_h, rnd_w: rnd_w , :]
        #     img_LQ = img_LQ[rnd_h: rnd_h, rnd_w: rnd_w , :]
        ...
        return {"GT": img_GT, "LQ": img_LQ, "LQ_clip": lq4clip,  "type": deg_type, "GT_path": GT_path,"LQ_path": LQ_path }
